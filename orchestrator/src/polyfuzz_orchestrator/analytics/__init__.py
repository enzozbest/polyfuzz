...
Public API:
    run_analytics(work_dir) -> AnalyticsResult
        Discovers campaign directories, collects per-campaign metrics,
        writes output files, and prints a terminal summary.
"""
...
def run_analytics(work_dir: Path) -> AnalyticsResult:
    """Run the analytics pipeline on an experiment directory.

    Discovers campaign_NNN directories, extracts per-campaign metrics,
    ...
    analytics_dir.mkdir(parents=True, exist_ok=True)
    write_all_analytics(analytics_dir, metrics_internal, skipped, work_dir)

    print_terminal_summary(metrics_internal, skipped)
