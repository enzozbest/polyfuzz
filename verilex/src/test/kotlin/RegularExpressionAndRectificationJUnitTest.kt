...
class RegularExpressionAndRectificationJUnitTest {
    @Test
    ...
    fun cfunAndLowering() {
        ...
        // mkeps on CFUN is illegal
        var threw = false
        try { cfun.mkeps() } catch (_: IllegalStateException) { threw = true }
        require(threw)
        // RANGE lowering
    ...
    fun starAndPlusBasics() {
        ...
        // mkeps on PLUS requires inner r to be nullable; use ONE to test this path
        require(PLUS(ONE).mkeps() is Plus)
    }
    ...
    fun recdBasics() {
        ...
        // mkeps only valid when inner regex is nullable; use ONE variant for this
        val rNullable = RECD("x", ONE)
        require(rNullable.mkeps() == Rec("x", Empty))
    }
    ...
    fun valueFlattenAndEnv() {
        ...
        // Plus flatten/env
        val p: Value = Plus(listOf(Chr('a'), Chr('b')))
        require(p.flatten() == "ab")
        require(p.env().isEmpty())
    }
    ...
    @Test
    fun injectorTopLevelRecWrapping() {
        // Cover Injector.inj outer Rec case (line 26): v is Rec should wrap recursively
        val a = CHAR('a').toCharFunctionFormat()
        val v = Rec("wrap", Empty)
        val res = Injector.inj(a, 'a', v)
        require(res == Rec("wrap", Chr('a')))
    }
}
