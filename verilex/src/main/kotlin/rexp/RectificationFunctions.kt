...
object Rectification {
    ...
    fun alt(f1: (Value) -> Value, f2: (Value) -> Value): (Value) -> Value = { v ->
        when (v) {
    ...
    fun seq(f1: (Value) -> Value, f2: (Value) -> Value): (Value) -> Value = { v ->
        when (v) {
    ...
    fun seq_Empty1(f1: (Value) -> Value, f2: (Value) -> Value): (Value) -> Value = { v ->
        Seq(f1(Empty), f2(v))
    }

    /** Rectification for concatenation when right simplifies to epsilon. */
    fun seq_Empty2(f1: (Value) -> Value, f2: (Value) -> Value): (Value) -> Value = { v ->
        Seq(f1(v), f2(Empty))
    }

    /** Rectification inside a recorded value. */
    fun recd(f: (Value) -> Value): (Value) -> Value = { v ->
        when (v) {
